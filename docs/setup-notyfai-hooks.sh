...
cat > "$HOOKS_JSON" << 'NOTYFAI_HOOKS_EOF'
{
  ...
  "hooks": {
    ...
    "beforeReadFile": [{ "command": ".cursor/hooks/notyfai-send.sh" }],
    "beforeTabFileRead": [{ "command": ".cursor/hooks/notyfai-send.sh" }],
    "afterFileEdit": [{ "command": ".cursor/hooks/notyfai-send.sh" }],
