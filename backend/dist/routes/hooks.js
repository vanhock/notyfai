...
const hookRateLimit = (0, express_rate_limit_1.default)({
    windowMs: 10 * 60 * 1000,
    max: 150,
    keyGenerator: (req) => {
        ...
        if (token && typeof token === "string") {
            const verified = (0, token_js_1.verifyAndGetInstanceId)(token);
            if (verified)
                return `instance:${verified.instanceId}`;
        }
        return (0, express_rate_limit_1.ipKeyGenerator)(req.ip ?? "0.0.0.0");
    },
    handler: (_req, res) => {
...
function trackAndCheckAbuse(instanceId) {
    const now = Date.now();
...
const pendingStaleBlockingTimeouts = new Map();
function cancelPending(executionId) {
...
function cancelStaleBlockingTimeout(executionId) {
 ...
 */
function resolveSemanticType(eventType, payload) {
    const base = (0, cursor_js_1.toSemanticType)(eventType);
    if (base === "agentBlocked" && cursor_js_1.PERMISSION_BEARING_HOOKS.has(eventType)) {
...
const AFTER_BLOCKING_HOOKS = new Set([
    "afterShellExecution",
    "afterMCPExecution",
    "afterFileEdit",
    "afterTabFileEdit",
]);
function blockingToolNameFromPayload(eventType, payload) {
 ...
 */
async function upsertExecution(instanceId, generationId, conversationId, status, initialStatus = "running", sessionId, metadata) {
    ...
    if (metadata?.prompt !== undefined)
        insertRow.prompt = metadata.prompt;
    if (metadata?.model !== undefined)
        insertRow.model = metadata.model;
    if (metadata?.workspace_roots !== undefined)
        insertRow.workspace_roots = metadata.workspace_roots;
    const { data: inserted, error: insertError } = await supabase_js_1.supabaseAdmin
        .from("agent_executions")
        .insert(insertRow)
        .select("id")
        .single();
    if (!insertError && inserted) {
        return { executionId: inserted.id, isNew: true };
    }
    ...
    const executionId = existing.id;
    // Forward-only: pending(0) → running(1) → blocked(2) → stopped(3)
    const rank = { pending: 0, running: 1, blocked: 2, stopped: 3 };
    const updates = {
        updated_at: new Date().toISOString(),
    };
    if (effectiveSessionId !== null)
        updates.session_id = effectiveSessionId;
    if ((rank[status] ?? -1) > (rank[existing.status] ?? -1)) {
        updates.status = status;
    }
    // Backfill metadata only when still null
    if (metadata?.model != null && existing.model == null)
    ...
    await supabase_js_1.supabaseAdmin.from("agent_executions").update(updates).eq("id", executionId);
    return { executionId, isNew: false };
}
function executionStatusForSemantic(semanticType) {
    switch (semanticType) {
        case "agentBlocked":
            return "blocked";
        case "agentStopped":
...
router.post("/cursor", hookRateLimit, async (req, res) => {
    const rawToken = req.query.token ?? req.headers["x-notyfai-token"];
    const token = Array.isArray(rawToken) ? rawToken[0] : rawToken;
    if (!token || typeof token !== "string") {
    ...
    const { data, error: fetchError } = await supabase_js_1.supabaseAdmin
    ...
    if (trackAndCheckAbuse(instanceId)) {
        ...
        await supabase_js_1.supabaseAdmin.from("cursor_instances").update({ revoked: true }).eq("id", instanceId);
        (0, notifications_js_1.sendPushNotification)(instance.user_id, "agentStopped", { status: "error", hook_event_name: "stop" }, `Security alert: hook URL for "${instance.name ?? instanceId.slice(0, 8)}" disabled due to suspicious activity.`).catch(() => { });
    ...
    const reason = cursor_js_1.PERMISSION_BEARING_HOOKS.has(eventType)
        ? `eventType=${eventType} permission=${perm ?? "(undefined)"} → semanticType=${semanticType}`
        : `eventType=${eventType} → semanticType=${semanticType}`;
    console.log("[hooks] %s | instanceId=%s userId=%s | %s", blockingLabel, instanceId, instance.user_id, reason);
    ...
    if (eventType === "sessionStart") {
        ...
        try {
            await upsertExecution(instanceId, sessionId, sessionId, "pending", "pending", sessionId);
        }
        catch (err) {
    ...
    if (!generationId) {
        ...
        await supabase_js_1.supabaseAdmin.from("cursor_events").insert({
            instance_id: instanceId,
            execution_id: null,
    ...
    if (eventType === "beforeSubmitPrompt") {
        ...
        let executionId;
        try {
            const result = await upsertExecution(instanceId, generationId, conversationId, "running", "running", conversationId, { prompt, model, workspace_roots: workspaceRoots });
    ...
    if (BEFORE_BLOCKING_HOOKS.has(eventType)) {
        targetStatus = "blocked";
    }
    else if (AFTER_BLOCKING_HOOKS.has(eventType)) {
        targetStatus = "running";
    }
    if (eventType === "stop") {
    ...
    let executionId;
    try {
        const result = await upsertExecution(instanceId, generationId, conversationId, targetStatus, "running", conversationId, Object.keys(metadata).length > 0 ? metadata : undefined);
        executionId = result.executionId;
    }
    catch (err) {
    ...
    if (BEFORE_BLOCKING_HOOKS.has(eventType)) {
        const blockingToolName = blockingToolNameFromPayload(eventType, payload);
        await supabase_js_1.supabaseAdmin
            .from("agent_executions")
            .update({
        ...
        if (semanticType !== "agentBlocked") {
            ...
            const timeout = setTimeout(async () => {
                pendingStaleBlockingTimeouts.delete(executionId);
                const { data: row } = await supabase_js_1.supabaseAdmin
                    .from("agent_executions")
                    .select("id, blocked_since, blocking_event_type, blocking_tool_name")
                    .eq("id", executionId)
                    .single();
                if (row?.blocked_since) {
                    await supabase_js_1.supabaseAdmin.from("agent_executions").update({ status: "blocked" }).eq("id", executionId);
                    ...
                    const stalePayload = {
                        ...
                        tool_name: (row.blocking_tool_name ?? payload.tool_name),
                        permission: "ask",
                    };
                    (0, notifications_js_1.sendPushNotification)(instance.user_id, "agentBlocked", stalePayload, instance.name, executionId).catch((err) => {
    ...
    else if (AFTER_BLOCKING_HOOKS.has(eventType) || eventType === "stop") {
        cancelStaleBlockingTimeout(executionId);
        const clearUpdate = {
            blocked_since: null,
        ...
        if (AFTER_BLOCKING_HOOKS.has(eventType)) {
            clearUpdate.status = "running";
        }
        await supabase_js_1.supabaseAdmin.from("agent_executions").update(clearUpdate).eq("id", executionId);
    }
    await supabase_js_1.supabaseAdmin
        .from("cursor_instances")
        .update({ last_event_at: new Date().toISOString() })
    ...
    if (semanticType === "agentStopped") {
        (0, notifications_js_1.sendPushNotification)(instance.user_id, semanticType, payload, instance.name, executionId).catch((err) => {
